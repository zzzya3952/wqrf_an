<template>
    <div>
        ...
        <form action="http://localhost:8000/upload_env_script/" method="post" enctype="multipart/form-data">
            <input name="upload_env_script" type="file" class="btn btn-outline-secondary btn-sm" style="border-radius: 5px 0 0 5px">
            <button type="submit" class="btn btn-secondary" style="border-radius: 0 5px 5px 0">上传</button>
        </form>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        watch:{
            now_env(){
                this.$emit('upe',{now_env:this.now_env,env_script:this.env_script})
            }
        }
